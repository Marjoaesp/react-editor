...
import { Playlist as PlaylistType, PlaylistItem } from './types';

interface PlaylistManagerProps {
  playlist: PlaylistType;
  onSave: (updated: PlaylistType) => void;
  onClose: () => void;
}

const PlaylistManager: React.FC<PlaylistManagerProps> = ({ playlist, onSave, onClose }) => {
  const [playlists, setPlaylists] = useState<PlaylistType[]>([]);
  const [playlistName, setPlaylistName] = useState<string>('');
  const [editingPlaylist, setEditingPlaylist] = useState<PlaylistType | null>(null);
  const [deletingPlaylist, setDeletingPlaylist] = useState<PlaylistType | null>(null);
  ...
  const publishPlaylist = async (playlist: PlaylistType) => {
    const zip = new JSZip();
