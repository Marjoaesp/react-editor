...
const PlaylistComponent: React.FC = () => {
  ...
  const publishPlaylist = async (playlist: Playlist) => {
    ...
    for (const item of playlist.items) {
      if (item.dataUrl.startsWith('blob:')) {
        ...
        zip.file(item.name, blob, { base64: true });
      } else {
  ...
  return (
    <div style={{ display: "flex", height: "100vh" }}>
      <SidebarMartin />
      <div className="p-4 flex flex-col items-start mt-12 w-full overflow-hidden">
        ...
        <div className="grid gap-4">
          {playlists.map((playlist) => (
            <div key={playlist.id} className="w-full p-4 border rounded hover:bg-gray-100">
