...
export default function CustomComponent() {
  ...
  React.useEffect(() => {
    if (scenes) {
      const calculatedMaxTime = scenes.reduce((previousVal, currentValue) => {
        return previousVal + (currentValue.duration || 0);
      }, 0);
  ...
  return (
    <Block
      $style={{
        ...
        width: "2px",
        bottom: "0px",
        cursor: "ew-resize", // Optional: Add cursor style for resize
      }}
    >
      <div
        onMouseDown={onStart} // Ensure that onMouseDown is applied to a child element, not Block itself
      >
        <Block
          id="EditorPlayHead"
          $style={{
